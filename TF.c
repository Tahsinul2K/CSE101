...
int token_count_for_doc = 0;
int token_count = 0, except_count = 0;
int set_flag = 0, tokenize_flag = 0;
int stem_count = 0;
char suffix[10];
char tokens_for_single_doc[MAX_DOCS][MAX_LEN];
char documents[MAX_DOCS][MAX_LEN];
char tokens[MAX_TOKENS][MAX_TOKEN_LEN];
char stop_words[NUM_STOP_WORDS][MAX_TOKEN_LEN] = {"the", "is", "a", "an", "and", "in", "of", "to"};

void preprocess_single_doc(int doc_id)
{
    for (int i = 0; i < MAX_TOKENS; i++)
    {
        tokens_for_single_doc[i][0] = '\0';
    }
    int j = 0;
    int k = 0, l = 0;
    // tokenizing a single documents
    token_count_for_doc = 0;
    while (1)
    {
        if (isalnum(documents[doc_id][j]))
        {
            tokens_for_single_doc[k][l] = documents[doc_id][j];
    ...
    // removing stop words
    int flag = 0, except_count_for_single_doc = 0;
    for (int i = 0; i < token_count_for_doc; i++)
    {
        flag = 0;
        for (int j = 0; j < NUM_STOP_WORDS; j++)
        {
            if (strcmp(tokens_for_single_doc[i], stop_words[j]) == 0)
            {
                flag = 1;
                break;
            }
        ...
        if (flag == 0)
        {
            strcpy(tokens_for_single_doc[except_count_for_single_doc], tokens_for_single_doc[i]);
            except_count_for_single_doc++;
        }
    }
    token_count_for_doc = except_count_for_single_doc;
    // stemming
    int stem_count_for_single_doc = 0;
    for (int i = 0; i < except_count_for_single_doc; i++)
    {
        int length = strlen(tokens_for_single_doc[i]);
        if (length >= 3 && strcmp(tokens_for_single_doc[i] + length - 3, "ing") == 0)
        {
            strcpy(tokens_for_single_doc[stem_count_for_single_doc], tokens_for_single_doc[i]);
            tokens_for_single_doc[stem_count_for_single_doc][length - 3] = '\0';
            stem_count_for_single_doc++;
            continue;
        }
        if (length >= 2 && strcmp(tokens_for_single_doc[i] + length - 2, "ed") == 0)
        {
            strcpy(tokens_for_single_doc[stem_count_for_single_doc], tokens_for_single_doc[i]);
            tokens_for_single_doc[stem_count_for_single_doc][length - 2] = '\0';
            stem_count_for_single_doc++;
            continue;
        }
        if (length >= 1 && strcmp(tokens_for_single_doc[i] + length - 1, "s") == 0)
        {
            strcpy(tokens_for_single_doc[stem_count_for_single_doc], tokens_for_single_doc[i]);
            tokens_for_single_doc[stem_count_for_single_doc][length - 1] = '\0';
            stem_count_for_single_doc++;
            continue;
        }
        strcpy(tokens_for_single_doc[stem_count_for_single_doc], tokens_for_single_doc[i]);
...
{
    preprocess_single_doc(doc_id);

    int occurance = 0;

    for (int i = 0; i < token_count_for_doc; i++)
    {
        if (strcmp(word, tokens_for_single_doc[i]) == 0)
        {
            occurance++;
        }
    }
    double tf = (occurance) / (token_count_for_doc) * 1.00;
    return tf;
}
void stem_all_tokens()
{
    ...
    for (int i = 0; i < except_count; i++)
    {
        ...
        if (strcmp(tokens[i] + length - 3, "ing") == 0)
        {
            strcpy(tokens[stem_count], tokens[i]);
            tokens[stem_count][length - 3] = '\0';
            stem_count++;
            continue;
        }
        if (strcmp(tokens[i] + length - 2, "ed") == 0)
        {
            strcpy(tokens[stem_count], tokens[i]);
            tokens[stem_count][length - 2] = '\0';
            stem_count++;
            continue;
        }
        if (strcmp(tokens[i] + length - 1, "s") == 0)
        {
            strcpy(tokens[stem_count], tokens[i]);
            tokens[stem_count][length - 1] = '\0';
            stem_count++;
            continue;
        }
        strcpy(tokens[stem_count], tokens[i]);
...
void remove_stop_words_all()
{
    ...
    for (int i = 0; i < token_count; i++)
    {
        ...
        if (flag == 0)
        {
            strcpy(tokens[except_count], tokens[i]);
            except_count++;
        }
    }
}
...
int main()
{
    ...
    while (1)
    {
        ...
        else if (strcmp(take_input, "tf") == 0)
        {
            char word[100];
            ...
            //    printf("Document %d: %lf\n",i+1,tf);
           // }

           for(int i = 0 ; i<token_count_for_doc; i++)
           {
            printf("%s\n",tokens_for_single_doc[i]);
           }
        }
    }
}